...
// Basic smoke tests for the treedb backend.
func TestTreeDBBasic(t *testing.T) {
	dir, err := os.MkdirTemp("", "treedb-basic-*")
	if err != nil {
		t.Fatalf("temp dir: %v", err)
	}
	defer os.RemoveAll(dir)

	db, err := NewDB("treedb-basic", TreeDBBackend, dir)
	if err != nil {
		t.Fatalf("new treedb db: %v", err)
	}
	defer db.Close()
...
func TestTreeDBBatch(t *testing.T) {
	dir, err := os.MkdirTemp("", "treedb-batch-*")
	if err != nil {
		t.Fatalf("temp dir: %v", err)
	}
	defer os.RemoveAll(dir)

	db, err := NewDB("treedb-batch", TreeDBBackend, dir)
	if err != nil {
		t.Fatalf("new treedb db: %v", err)
	}
	defer db.Close()
