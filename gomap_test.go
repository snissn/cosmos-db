...
// Basic smoke tests for the gomap backend to mirror coverage given to other backends.
func TestGomapBasic(t *testing.T) {
	dir, err := os.MkdirTemp("", "gomap-db-*")
	if err != nil {
		t.Fatalf("temp dir: %v", err)
	}
	defer os.RemoveAll(dir)

	db, err := NewDB("gomap-basic", GomapBackend, dir)
	if err != nil {
		t.Fatalf("new gomap db: %v", err)
	}
	defer db.Close()
...
func TestGomapBatch(t *testing.T) {
	dir, err := os.MkdirTemp("", "gomap-batch-*")
	if err != nil {
		t.Fatalf("temp dir: %v", err)
	}
	defer os.RemoveAll(dir)

	db, err := NewDB("gomap-batch", GomapBackend, dir)
	if err != nil {
		t.Fatalf("new gomap db: %v", err)
	}
	defer db.Close()
