...
// Basic smoke tests for the cachedgeminitreedb backend.
func TestCachedGeminiTreeDBBasic(t *testing.T) {
	dir, err := os.MkdirTemp("", "cachedgeminitreedb-basic-*")
	if err != nil {
		t.Fatalf("temp dir: %v", err)
	}
	defer os.RemoveAll(dir)

	db, err := NewDB("cachedgeminitreedb-basic", CachedGeminiTreeDBBackend, dir)
	if err != nil {
		t.Fatalf("new cachedgeminitreedb db: %v", err)
	}
	defer db.Close()
...
func TestCachedGeminiTreeDBBatch(t *testing.T) {
	dir, err := os.MkdirTemp("", "cachedgeminitreedb-batch-*")
	if err != nil {
		t.Fatalf("temp dir: %v", err)
	}
	defer os.RemoveAll(dir)

	db, err := NewDB("cachedgeminitreedb-batch", CachedGeminiTreeDBBackend, dir)
	if err != nil {
		t.Fatalf("new cachedgeminitreedb db: %v", err)
	}
	defer db.Close()
